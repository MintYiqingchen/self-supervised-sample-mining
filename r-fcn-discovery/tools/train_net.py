...
import _init_paths
from fast_rcnn.train import get_training_roidb, train_net, SolverWrapper, update_training_roidb,filter_roidb, ClassController
...
if __name__ == '__main__':
                    ...
                    if(np.sum(y==1)==1 and np.where(y==1)[0]!=0): # not background
                       # add Imgae Cross Validation
                        if args.enable_icv:
                         ...
                         #   cross_validate = image_cross_validation(model,roidb,labeledidx,curr_roidb,pre_box,pre_cls,resize=False)
                            
                            if args.enable_mmv:
                                mmv_validate = 0
                                for lasted_index in range(mmv_k):
                                    lasted_index = -(lasted_index+1)
                                    mmv_model_name = trained_models[lasted_index]
                                    mmv_model_path = os.path.join(output_dir,mmv_model_name)
                                    mmv_model = load_model(protopath,mmv_model_path)
                                    print ('mmv_model_name:',mmv_model_name)
                                    cross_validate = image_cross_validation(mmv_model,roidb,labeledidx,curr_roidb,pre_box,pre_cls,resize=False,flipped=False)
                                    mmv_validate += cross_validate
                                if mmv_validate > (mmv_k/2):
                                    img_boxes.append(box)
                                    cls.append(np.where(y==1)[0])
                                else:
                                    continue
                            else:
                                cross_validate = image_cross_validation(model,roidb,labeledidx,curr_roidb,pre_box,pre_cls,resize=False,flipped=False)
                                if cross_validate:
                                    img_boxes.append(box)
                                    cls.append(np.where(y==1)[0])
                                else:
	...
	classController.controlClass(iters_sum) 
        ...
        loopcounter += 1
        if iters_sum<=tao:
            clslambda = 0.9 * clslambda+0.1*cls_loss_sum/count_box_num
            cls_loss_sum = 0.0

        # add the labeled samples to finetune W
        train_iters = min(20000 ,len(train_roidb)*10-train_iters)
